...
def get_explosive_plays(player_name, team_name):
    """
    Scans Play-by-Play for:
    - Runs > 15 yards
    - Passes/Catches > 20 yards
    """
    print(f"💥 Scanning for Explosive Plays...", end="\r")
    explosive_stats = {} 
    url = "https://api.collegefootballdata.com/plays"
    
    # Scan Regular Season + Postseason
    # Combining loops for efficiency
    tasks = [
        {"seasonType": "regular", "weeks": range(1, 16)},
        {"seasonType": "postseason", "weeks": [1]} # API ignores week for postseason usually, but good practice
    ]
    ...
    for task in tasks:
        ...
        # If regular, iterate weeks. If postseason, just one call usually needed.
        loop_iter = weeks if sType == 'regular' else [1]
        
        for week in loop_iter:
            try:
                params = { "year": CURRENT_YEAR, "team": team_name, "seasonType": sType }
                if sType == 'regular': params['week'] = week
                ...
                for play in plays:
                    ...
                    if player_name in play.get('playText', ''):
                        ...
                        # EXPLOSIVE RUN: Rush > 15
                        if "Rush" in p_type and yards > 15:
                            explosive_stats[g_id]['ExpRun'] += 1
                            
                        # EXPLOSIVE PASS/REC: > 20
                        # Whether throwing (QB) or catching (WR), it's the same play type
...
def get_stats_profile(position):
    """
    Adjusts columns based on position.
    QB gets 'ExpPass', others get 'ExpRec'.
    """
    if position == 'QB':
        return {
            'cols': ['Date', 'Opp', 'C/ATT', 'PasYds', 'PasTD', 'Int', 'ExpPass', 'Car', 'RusYds', 'RusTD', 'ExpRun']
        }
    else:
        return {
            'cols': ['Date', 'Opp', 'Car', 'RusYds', 'RusTD', 'ExpRun', 'Rec', 'RecYds', 'RecTD', 'ExpRec']
        }
...
def get_player_game_stats(player_data):
    ...
    print(f"\n🏈 --- MASTER SCOUT REPORT: {name.upper()} ({pos}) ---")
        ...
        try:
            ...
            for game in games_data:
                ...
                stats = {
                    ...
                    'ExpRec': exp_stats['ExpPass']   # WR Label (same data source)
                }
                
                player_found = False
                for t in game.get('teams', []):
                    if (t.get('school') == team) or (t.get('team') == team):
                            ...
                            for stat_type in cat.get('types', []):
                                ...
                                for ath in stat_type.get('athletes', []):
                                    if ath.get('name') == name:
                                        ...
                                        if cat_name == 'passing':
                                            if s_label == 'C/ATT': stats['C/ATT'] = val_str
    ...
    if all_rows:
        ...
        cols = profile['cols']
        # Dynamic Header
        # Adjust spacing for QB columns (PasYds, etc) vs WR columns
        header_str = ""
        for c in cols:
        ...
        print("="*len(header_str))
        if pos == 'QB':
            print("* ExpPass = Throws > 20 yards")
            print("* ExpRun  = Runs > 15 yards")
        else:
            print("* ExpRec = Catches > 20 yards")
            print("* ExpRun = Runs > 15 yards")
        
        fname = f"{name.replace(' ', '_')}_{pos}_master_2024.csv"
...
if __name__ == "__main__":
    while True:
        ...
        if p_data: get_player_game_stats(p_data)