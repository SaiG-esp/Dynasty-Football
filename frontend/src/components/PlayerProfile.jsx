...
const PlayerProfile = ({ player, onBack }) => {
  ...
  if (!player)
    return (
      <div className="profile-container">
        <div style={{ padding: 40 }}>Loading Player...</div>
      </div>
    );
  ...
  const loadAdvancedStats = async () => {
    ...
    try {
      ...
      const message = err instanceof ApiError ? err.message : 'Error loading advanced stats.';
      setAdvError(message);
    } finally {
          ...
          <div className="hero-details">
            ...
            <div className="hero-vitals-row">
              <span className="vital-item">
                <span className="vital-label">CLASS</span> {age}
              </span>
              <span className="vital-divider">|</span>
              <span className="vital-item">
                <span className="vital-label">HEIGHT</span> {height}
              </span>
              <span className="vital-divider">|</span>
              <span className="vital-item">
                <span className="vital-label">WEIGHT</span> {weight}
              </span>
              <span className="vital-divider">|</span>
              <span className="vital-item school-highlight">{player.school}</span>
            </div>
            <div className="hero-badges-row">
              <span className="pos-badge" style={{ backgroundColor: getPosBadgeColor(pos) }}>
                {pos}
              </span>
            </div>
          </div>
        ...
        {activeTab === 'GAME LOG' && (
          <div className="data-card">
            <div className="card-header">
              <h3>2025 College Season Stats</h3>
            </div>
            <table className="profile-table">
              ...
              <tbody>
                <tr
                  className="highlight-row clickable-row"
                  onClick={() => setSeasonExpanded(!seasonExpanded)}
                >
                  <td align="left">
                    <span className={`dropdown-caret ${seasonExpanded ? 'open' : ''}`}>▶</span> 2025
                  </td>
                  <td align="left" className="school-text">
                    Season Totals
                  </td>
                  {isQB && (
                    <>
                      <td>-</td>
                      <td className="stat-pop">{stats.s1 || 0}</td>
                      <td className="stat-pop">{stats.s2 || 0}</td>
                      <td>-</td>
                      <td>-</td>
                    </>
                  )}
                ...
                {seasonExpanded &&
                  gameLog.map((game, i) => (
                    <tr key={i} className="game-log-row">
                      <td align="left" style={{ paddingLeft: '30px' }}>
                        Wk {game.week}
                      </td>
                      <td align="left" style={{ color: '#ccc' }}>
                        vs {game.opponent}
                      </td>
                      {isQB && (
                        <>
                          <td>{game['C/ATT']}</td>
                          <td style={{ color: '#fff' }}>{game.PasYds}</td>
                          <td style={{ color: '#fff' }}>{game.PasTD}</td>
                          <td style={{ color: '#f44' }}>{game.Int}</td>
                          <td>{game.RusYds}</td>
                        </>
                      )}
                      {isRB && (
                        <>
                          <td>{game.Car}</td>
                          <td style={{ color: '#fff' }}>{game.RusYds}</td>
                          <td>-</td>
                          <td style={{ color: '#fff' }}>{game.RusTD}</td>
                          <td>{game.Rec}</td>
                        </>
                      )}
                      {!isQB && !isRB && (
                        <>
                          <td>{game.Rec}</td>
                          <td style={{ color: '#fff' }}>{game.RecYds}</td>
                          <td>-</td>
                          <td style={{ color: '#fff' }}>{game.RecTD}</td>
                        </>
                      )}
                    </tr>
                  ))}
          ...
          <div className="data-card">
            ...
            >
              ...
              {!advStats && !isLoadingAdv && (
                <button className="analyze-btn" onClick={loadAdvancedStats}>
                  Load Advanced Data
                </button>
              )}
            </div>
                  ...
                  <tbody>
                    {advStats.map((row) => (
                      <tr key={`${row.id}-bet`}>
                        ...
                        <td align="left" style={{ fontSize: '0.8rem' }}>
                          {row.spread}
                        </td>
                        <td style={{ color: row.winProb > 50 ? '#00C853' : '#E03A3E' }}>
                          {row.winProb}%
                        </td>
                        <td>{row.overUnder}</td>
                        ...
                        <td>
                          <span
                            className={`ctx-tag ${row.context.replace(' ', '-').toLowerCase()}`}
                          >
                            {row.context}
                          </span>
                        </td>
            ...
            {!isLoadingAdv && !advStats && !advError && (
              <div className="empty-state-small">
                Click "Load Advanced Data" to fetch Red Zone & Betting Analytics.
              </div>
            )}
          </div>
        )}
